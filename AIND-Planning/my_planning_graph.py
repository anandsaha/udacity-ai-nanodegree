...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            valid = True
            for prereq in action_node.prenodes:
                if prereq not in self.s_levels[level]:
                    valid = False

            if valid:
                for parent in self.s_levels[level]:
                    parent.children.add(action_node)
                    action_node.parents.add(parent)

                self.a_levels[level].append(action_node)
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for action1 in node_s1.parents:
            for action2 in node_s2.parents:
                if (action1 != action2) and (action1.is_mutex(action2)):
                    count += 1
    ...
    def h_levelsum(self) -> int:
        ...
        for g in self.problem.goal:
            ...
            for lindex in range(num_levels):
                for state in self.s_levels[lindex]:
